...
class two_m_playback(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, center_freq=146500000, in_file="/tmp/2m_capture.iq"):
        ...
        self.settings = Qt.QSettings("GNU Radio", "two_m_playback")
        ...
        self.osmosdr_sink_0.set_center_freq(146.520e6, 0)
...
def main(top_block_cls=two_m_playback, options=None):
    ...
    def eof_watcher():
        # osmosdr_sink is a hardware clock master and keeps the scheduler alive
        # indefinitely, so tb.wait() never returns on its own. Instead, sleep
        # for the exact playback duration (derived from sample count) then stop.
